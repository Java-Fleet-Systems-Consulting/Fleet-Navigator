<template>
  <div class="min-h-screen p-6 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
    <!-- Loading State -->
    <div v-if="loading" class="flex items-center justify-center h-full">
      <div class="text-center">
        <div class="animate-spin rounded-full h-16 w-16 border-b-2 border-fleet-orange-500 mx-auto"></div>
        <p class="text-gray-400 mt-4">Lade Maat-Details...</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="flex items-center justify-center h-full">
      <div class="text-center">
        ...
        <h2 class="text-xl font-bold text-white mb-2">Fehler beim Laden</h2>
        <p class="text-gray-400">{{ error }}</p>
        <button
          @click="$router.push({ name: 'fleet-mates' })"
          class="mt-4 px-4 py-2 bg-fleet-orange-500 hover:bg-fleet-orange-600 text-white rounded-lg transition-colors"
        >
    ...
    <!-- Mate Detail Content -->
    <div v-else-if="mate" class="w-full max-w-7xl mx-auto">
      <div class="
        bg-gray-900
      ...
      ">
        ...
        <div class="flex gap-2 px-6 pt-4 border-b border-gray-700/50 bg-gray-900/50">
          ...
          <button
            @click="activeTab = 'fleetcode'"
            class="
              px-4 py-3 rounded-t-lg font-semibold text-sm
              transition-all duration-200
              flex items-center gap-2
            "
            :class="activeTab === 'fleetcode'
        ...
        <div class="p-6 min-h-[600px]">
              ...
              <div class="space-y-3">
                ...
                <div v-if="analyzing && readingProgress < 100" class="space-y-2">
                  <div class="flex items-center justify-between text-xs text-gray-400">
                    <span>Log-Datei wird gelesen...</span>
                    <span class="font-mono">{{ readingProgress }}%</span>
                  </div>
                  <div class="w-full bg-gray-700 rounded-full h-2 overflow-hidden">
                    <div
                      class="h-full bg-gradient-to-r from-fleet-orange-500 to-orange-600 transition-all duration-300"
                ...
                <div v-if="analyzing && readingProgress >= 100" class="space-y-2 p-3 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-lg border border-blue-500/30">
                  <div class="flex items-center gap-3">
                    <div class="relative">
                      <div class="w-8 h-8 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin"></div>
                      <div class="absolute inset-0 flex items-center justify-center">
                        <SparklesIcon class="w-4 h-4 text-blue-400 animate-pulse" />
                      </div>
                    </div>
                    <div class="flex-1">
                      <div class="text-sm font-semibold text-blue-300">🤖 KI analysiert Log-Datei...</div>
                      <div class="text-xs text-gray-400 mt-0.5">Dies kann bei großen Logs einige Minuten dauern</div>
                    </div>
                  </div>
                  <!-- Animated progress bar -->
                  <div class="w-full bg-gray-700 rounded-full h-1 overflow-hidden">
                    <div class="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-blue-500 animate-pulse" style="width: 100%"></div>
                  </div>
                </div>
          ...
          <!-- FleetCode Tab -->
          <div v-if="activeTab === 'fleetcode'">
            <FleetCodeTab
              :mateId="mate.mateId"
              :mateOnline="mate.status === 'ONLINE'"
...
import MateTerminal from '../../components/MateTerminal.vue'
import FleetCodeTab from '../../components/FleetCodeTab.vue'
...
const loadMateData = async () => {
  try {
    const mateId = route.params.mateId

    // Load mate details
    const mateResponse = await axios.get(`/api/fleet-mate/mates/${mateId}`)
    mate.value = mateResponse.data

    // Load stats
    try {
      const statsResponse = await axios.get(`/api/fleet-mate/mates/${mateId}/stats`)
    ...
    console.error('Failed to load mate:', err)
    error.value = 'Maat konnte nicht geladen werden'
    loading.value = false
  }
}

// Load available models from LLMProviderService
async function loadAvailableModels() {
  ...
  try {
    ...
    availableModels.value = [
      { name: 'qwen2.5-coder-3b-instruct-q4_k_m.gguf', size: 'Unknown' },
      { name: 'mistral-7b-instruct-v0.2.Q4_K_M.gguf', size: 'Unknown' },
      { name: 'qwen2.5:7b', size: 'Unknown' }
    ]
  } finally {
...
async function pingMate() {
  pinging.value = true
  try {
    await axios.post(`/api/fleet-mate/mates/${mate.value.mateId}/ping`)
    success('Ping erfolgreich gesendet')
  } catch (err) {
    console.error('Failed to ping mate:', err)
    errorToast('Ping fehlgeschlagen')
  } finally {
    setTimeout(() => {
      pinging.value = false
    }, 500)
...
async function startLogAnalysis() {
  ...
  try {
    ...
    const response = await axios.post(
      `/api/fleet-mate/mates/${mate.value.mateId}/analyze-log`,
      {
    ...
    eventSource.addEventListener('progress', (event) => {
      ...
      console.log('[SSE] progress:', readingProgress.value + '%')
    })

    eventSource.addEventListener('start', (event) => {
      console.log('[SSE] start event received:', event.data)
      const data = JSON.parse(event.data)
      readingProgress.value = 100  // Log reading completed
...
`
      ...
      eventSource.close()
      analyzing.value = false
      errorToast('Log-Analyse unterbrochen')
    })
    ...
    analysisOutput.value += `\n✗ Fehler: ${err.message}\n`
    analyzing.value = false
    errorToast('Fehler beim Starten der Analyse')
  }
}

function navigateToSettings(tab) {
  router.push({ name: 'settings', query: { tab: tab || 'provider' } })
}
...
function getCoreTemp(coreIndex) {
  if (!stats.value?.temperature?.per_core) return null
  return stats.value.temperature.per_core[coreIndex]
}
...
onMounted(async () => {
  ...
  intervalId = setInterval(async () => {
    if (mate.value) {
      try {
        const mateId = route.params.mateId
        const [mateResponse, statsResponse] = await Promise.all([
          axios.get(`/api/fleet-mate/mates/${mateId}`),
          axios.get(`/api/fleet-mate/mates/${mateId}/stats`).catch(() => ({ data: null }))
        ])
        mate.value = mateResponse.data
        stats.value = statsResponse.data
      } catch (err) {
        console.error('Failed to refresh mate data:', err)
      }
    }
  }, 5000)
})
