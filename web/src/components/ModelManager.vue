<template>
  ...
  <!-- Create Expert Modal (for editing) -->
  <CreateExpertModal
  ...
  <!-- Expert Creation Wizard (for new experts) -->
  <ExpertCreationWizard
    :show="showExpertWizard"
    @close="closeExpertWizard"
    @created="onExpertCreatedFromWizard"
  />
  ...
  <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        <!-- Header with Gradient - NICHT SCHRUMPFEN -->
        <div class="
        ...
        <div class="border-b border-gray-200/50 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-900/50 flex-shrink-0">
          <div class="flex">
            ...
            >
              ...
              <span>{{ $t('modelManager.installedModels') }}</span>
            ...
            >
              ...
              <span>{{ $t('modelManager.availableModels') }}</span>
          ...
          <div class="flex items-center justify-between">
            <div class="flex-1">
              <div class="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $t('modelManager.currentModel') }}</h3>
                  <p class="text-sm text-gray-600 dark:text-gray-400">{{ chatStore.selectedModel || $t('modelManager.noModelSelected') }}</p>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-4">
        ...
        <div v-if="activeTab === 'installed'" class="border-b border-gray-200/50 dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/30">
          <div class="flex px-4">
            ...
            >
              ...
              <span>{{ $t('modelManager.experts') }}</span>
            ...
            >
              <SparklesIcon class="w-4 h-4" />
              <span>{{ $t('modelManager.customModels') }}</span>
      ...
      <!-- Installed Models List - Shown based on sub-tab - MUSS SCROLLEN! -->
      <div v-if="activeTab === 'installed'" class="flex-1 overflow-y-auto p-6 min-h-0">
        <!-- Loading state -->
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    <!-- Type Badge -->
                    <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400">
                      ⚡ GGUF
                    </span>
                  </div>
                  ...
                  <div class="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-500">
                      Erstellt: {{ formatDateLong(model.createdAt) }}
                    </p>
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-2 ml-4 min-w-[200px]">
                  ...
                  <button
                    @click="deleteCustomModel(model.id, model.name, model.isGgufConfig)"
                    class="
                      w-full px-4 py-2 rounded-lg
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <div class="flex flex-wrap items-center gap-2 mb-2 text-sm">
                    <!-- Size Badge -->
                    <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded border border-blue-300 dark:border-blue-700 font-medium">
                      📦 {{ formatSize(model.size) }}
                    </span>
                    <!-- Category Badge -->
                    <span v-if="getModelCategory(model.name) === 'coder'" class="px-2 py-1 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 rounded">
                      💻 Code
                    </span>
                    <span v-else-if="getModelCategory(model.name) === 'vision'" class="px-2 py-1 bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 rounded">
                      👁️ Vision
                    </span>
                    <span v-else class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded">
                      💬 Chat
                    </span>
                    <!-- License Badge -->
                    <span v-if="model.license || model.registry_entry?.license" class="px-2 py-1 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded text-xs">
                      📜 {{ model.license || model.registry_entry?.license }}
                    </span>
                  </div>

                  <!-- Description -->
                  <p v-if="model.description || model.registry_entry?.description" class="text-sm text-gray-700 dark:text-gray-300 mb-2 leading-relaxed">
                    {{ model.description || model.registry_entry?.description }}
                  </p>

                  <!-- Metadata Grid -->
                  <div class="grid grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <div v-if="model.registry_entry?.huggingface_repo">
                      <span class="text-gray-500">👤 Repo:</span>
                      <span class="ml-1 text-gray-800 dark:text-gray-200">{{ model.registry_entry.huggingface_repo.split('/')[0] }}</span>
                    </div>
                    <div v-if="model.registry_entry?.downloads">
                      <span class="text-gray-500">⬇️ Downloads:</span>
                      <span class="ml-1 text-gray-800 dark:text-gray-200">{{ formatDownloads(model.registry_entry.downloads) }}</span>
                    </div>
                    <div v-if="model.releaseDate || model.registry_entry?.release_date">
                      <span class="text-gray-500">📅 Release:</span>
                      <span class="ml-1 text-gray-800 dark:text-gray-200">{{ model.releaseDate || model.registry_entry?.release_date }}</span>
                    </div>
                    <div v-if="model.registry_entry?.context_window">
                      <span class="text-gray-500">📏 Context:</span>
                      <span class="ml-1 text-gray-800 dark:text-gray-200">{{ model.registry_entry.context_window }}</span>
                    </div>
                    <div v-if="model.specialties || model.registry_entry?.strengths" class="col-span-2">
                      <span class="text-gray-500">⭐ Stärken:</span>
                      <span class="ml-1 text-gray-800 dark:text-gray-200">{{ model.specialties || model.registry_entry?.strengths }}</span>
                    </div>
                  </div>
                </div>

                <div class="flex flex-col gap-2 ml-4 min-w-[200px]">
                  <!-- Info Button - Link to HuggingFace -->
                  <a
                    :href="getModelHuggingFaceUrl(model)"
                    target="_blank"
        ...
        <div v-else-if="installedSubTab === 'experts'">
          ...
          <div v-if="!chatStore.experts?.length" class="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    <div
                      ...
                      class="w-10 h-10 rounded-lg overflow-hidden border-2 border-indigo-400/50 shadow-sm flex-shrink-0"
                    >
                      <img
                    ...
                    <div
                      v-else
                      class="p-2 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex-shrink-0"
                    >
      ...
      <div v-else-if="activeTab === 'available'" class="flex-1 overflow-y-auto p-6 min-h-0">
                ...
                >
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                      ...
                      <!-- Modellgröße prominent anzeigen -->
                      <div v-if="model.modelSize" class="mt-2">
                        <span class="px-3 py-1.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded-lg border border-blue-300 dark:border-blue-700 font-medium text-sm">
                    ...
                    <div class="flex flex-col gap-2 ml-3">
                      <button
                        v-if="!isModelStoreInstalled(model)"
                        class="px-4 py-2 bg-fleet-orange-500 hover:bg-fleet-orange-600 text-white rounded-lg transition-colors whitespace-nowrap font-medium"
                      ...
                      <span
                        v-else
                        class="px-4 py-2 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 rounded-lg font-medium text-center"
                      >
                        ✅ Installiert
                      </span>
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        class="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-600 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 rounded-lg transition-colors whitespace-nowrap font-medium text-center text-sm"
                        @click.stop
                      >
                        ℹ️ Info
                      </a>
                  ...
                  <div class="grid grid-cols-2 gap-3 mb-3 text-sm">
                    ...
                    <div v-if="model.createdAt">
                      ...
                      <span class="ml-2 text-gray-900 dark:text-white">{{ formatDateLong(model.createdAt) }}</span>
                    </div>

                    <!-- Last Modified -->
                    <div v-if="model.lastModified">
                      <span class="text-gray-600 dark:text-gray-400">🔄 Aktualisiert:</span>
                      <span class="ml-2 text-gray-900 dark:text-white">{{ formatDateLong(model.lastModified) }}</span>
                    </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-lg shadow-2xl border-4 border-fleet-orange-500">
        ...
        <div v-if="!isDownloading">
          ...
          <div class="text-xs text-gray-500 dark:text-gray-400 mb-4">
            ...
            <p class="mt-2">Weitere Modelle: <a href="https://huggingface.co/models?library=gguf" target="_blank" class="text-fleet-orange-500 hover:underline">HuggingFace GGUF Models</a></p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div v-if="selectedModelDetails" class="space-y-3 text-sm">
          ...
          <div v-if="selectedModelDetails.releaseDate">
            ...
            <p class="text-gray-600 dark:text-gray-400">{{ formatDateLong(selectedModelDetails.releaseDate) }}</p>
          </div>
  ...
  <!-- Create Custom Model Wizard (ersetzt das alte Modal) -->
  <CustomModelWizard
...
import CustomModelWizard from './CustomModelWizard.vue'
...
const { confirm: showConfirm, confirmDelete: showDeleteConfirm } = useConfirmDialog()
...
const installedSubTab = ref('experts')  // Sub-tab for installed models: experts, downloaded, custom
...
function getModelCategory(modelName) {
  // Handle undefined or null model names - return 'general' silently to avoid console spam
  if (!modelName) {
    return 'general'
  }
  ...
  // If no model found, still try to categorize by name alone
  if (!model) {
    // Direct name-based categorization as fallback
    const nameLower = modelName.toLowerCase()
    if (nameLower.includes('llava') || nameLower.includes('vision') || nameLower.includes('moondream')) {
      return 'vision'
    }
    if (nameLower.includes('coder') || nameLower.includes('codellama') || nameLower.includes('deepseek-coder')) {
      return 'coder'
    }
    return 'general'
  }
...
// Helper function: Get HuggingFace URL for a model
function getModelHuggingFaceUrl(model) {
  // 1. If model has registry_entry with HuggingFaceRepo, use that
  if (model.registry_entry?.huggingface_repo) {
    return `https://huggingface.co/${model.registry_entry.huggingface_repo}`
  }

  // 2. If model has huggingFaceRepo directly
  if (model.huggingFaceRepo) {
    return `https://huggingface.co/${model.huggingFaceRepo}`
  }

  // 3. Try to find matching registry entry by filename
  const registryModel = availableModels.value.find(m =>
    m.filename === model.name ||
    model.name?.includes(m.filename) ||
    m.filename?.includes(model.name?.replace('.gguf', ''))
  )
  if (registryModel?.huggingface_repo) {
    return `https://huggingface.co/${registryModel.huggingface_repo}`
  }

  // 4. Fallback: Generate search URL based on model name
  // Clean model name for search (remove .gguf, quantization suffixes, etc.)
  let searchTerm = model.name || ''
  searchTerm = searchTerm
    .replace('.gguf', '')
    .replace(/[-_]q[0-9k]+[_]?[a-z]*/gi, '') // Remove q4_k_m, q5_0, etc.
    .replace(/[-_](fp16|f16|f32|bf16)/gi, '') // Remove precision suffixes
    .replace(/[-_](small|medium|large|mini)/gi, '') // Keep size indicators
    .trim()

  return `https://huggingface.co/models?search=${encodeURIComponent(searchTerm)}+gguf`
}
...
const downloadedModels = computed(() => {
  ...
  // Filter out models without a valid name
  let filtered = models.value.filter(m => m && m.name && getModelCategory(m.name) !== 'custom')

  // Apply type filter (coder, vision, general)
  if (downloadedFilter.value !== 'all') {
    filtered = filtered.filter(m => m.name && getModelCategory(m.name) === downloadedFilter.value)
  }
...
const coderModels = computed(() => models.value.filter(m => m && m.name && getModelCategory(m.name) === 'coder'))
const visionModels = computed(() => models.value.filter(m => m && m.name && getModelCategory(m.name) === 'vision'))
const generalModels = computed(() => models.value.filter(m => m && m.name && getModelCategory(m.name) === 'general'))
...
// Check if Model Store model is already installed (GGUF)
function isModelStoreInstalled(model) {
  if (!model || !model.filename) return false
  return models.value.some(m => {
    const installedName = m.name || m
    return installedName === model.filename ||
           installedName.includes(model.filename) ||
           model.filename.includes(installedName)
  })
}
...
const downloadStatus = ref('') // Status: 'downloading', 'completed', 'error'
...
// Load models from llama-server
async function loadModels() {
  isLoading.value = true
  try {
    console.log('Loading llama-server models...')
    ...
    console.log(`Loaded ${models.value.length} models`)
  } catch (error) {
...
async function selectExpert(expert) {
  try {
    ...
    // Nur Benachrichtigung anzeigen wenn beim Experten aktiviert (default: true)
    if (expert.showSwitchNotification !== false) {
      success(`Gewechselt zu ${expert.name}, ${expert.role}`)
    }
    emit('close')
  } catch (error) {
...
// Open create expert modal (now opens Wizard instead)
async function openCreateExpertModal() {
  // Open the new Expert Creation Wizard
  showExpertWizard.value = true
}

// Close expert wizard
function closeExpertWizard() {
  showExpertWizard.value = false
}

// Handle expert created from wizard
async function onExpertCreatedFromWizard(expert) {
  // Reload experts in chatStore
  await chatStore.loadModels()
  success(`Experte "${expert?.name || 'Neuer Experte'}" erstellt!`)
}

// Open old expert modal for editing
async function openCreateExpertModalForEditing() {
  // Load available models for the expert
  try {
    const response = await api.getAvailableModels()
    // API gibt {current_model, models: [string]} zurück
    // CreateExpertModal erwartet [{name, displayName}]
    const modelList = response.models || response || []
    availableModelsForExpert.value = modelList.map(m => {
      if (typeof m === 'object' && m.name) return m
      return { name: m, displayName: m }
    })
  } catch (e) {
    console.warn('Could not load models for expert creation:', e)
    availableModelsForExpert.value = generalModels.value.map(m => ({ name: m.name, displayName: m.name }))
  }
...
async function deleteExpertFromModal(expert) {
  const confirmed = await showDeleteConfirm(expert.name, 'Der Experte und alle seine Blickwinkel werden gelöscht.')
  if (!confirmed) return
...
// formatDate importiert aus useFormatters.js als formatDateLong
...
/**
 * Load custom GGUF model configurations from database
 */
async function loadCustomModels() {
  try {
    ...
    // Load GGUF Model Configs
    try {
      const ggufConfigs = await api.getAllGgufModelConfigs()
      if (ggufConfigs && ggufConfigs.length > 0) {
        ggufConfigs.forEach(config => {
          allCustomModels.push({
            ...
            displayType: 'GGUF Konfiguration',
            isGgufConfig: true
          })
        })
      }
    } catch (e) {
      console.warn('Could not load GGUF model configs:', e)
    }

    customModels.value = allCustomModels
    console.log(`Loaded ${allCustomModels.length} GGUF model configurations`)
  } catch (error) {
...
/**
 * Delete GGUF model configuration
 */
async function deleteCustomModel(id, name, isGgufConfig = true) {
  const confirmed = await showDeleteConfirm(name, 'Die GGUF-Konfiguration wird gelöscht. Das Basis-Modell bleibt erhalten.')
  if (!confirmed) return

  try {
    await api.deleteGgufModelConfig(id)
    success(`GGUF Konfiguration "${name}" gelöscht`)
    loadModels()
    loadCustomModels()
  } catch (error) {
    console.error('Failed to delete custom model:', error)
    errorToast(`Fehler beim Löschen: ${error.message}`)
  }
}
...
async function searchHuggingFace() {
  ...
  try {
    ...
    hfSearchResults.value = results || [] // Fallback auf leeres Array
    console.log('HuggingFace search results:', hfSearchResults.value.length)
  } catch (error) {
    console.error('HuggingFace search failed:', error)
    errorToast('Suche fehlgeschlagen: ' + error.message)
  } finally {
...
async function loadPopularHF() {
  ...
  try {
    ...
    hfSearchResults.value = results || [] // Fallback auf leeres Array
    console.log('Popular HuggingFace models:', hfSearchResults.value.length)
  } catch (error) {
    console.error('Failed to load popular models:', error)
    errorToast('Laden fehlgeschlagen: ' + error.message)
  } finally {
    isSearchingHF.value = false
  }
}
...
async function loadGermanHF() {
  ...
  try {
    ...
    hfSearchResults.value = results || [] // Fallback auf leeres Array
    console.log('German HuggingFace models:', hfSearchResults.value.length)
  } catch (error) {
...
async function loadInstructHF() {
  ...
  try {
    ...
    hfSearchResults.value = results || [] // Fallback auf leeres Array
    console.log('Instruct HuggingFace models:', hfSearchResults.value.length)
  } catch (error) {
...
async function loadCodeHF() {
  ...
  try {
    ...
    hfSearchResults.value = results || [] // Fallback auf leeres Array
    console.log('Code HuggingFace models:', hfSearchResults.value.length)
  } catch (error) {
...
async function loadVisionHF() {
  ...
  try {
    ...
    hfSearchResults.value = results || [] // Fallback auf leeres Array
    console.log('Vision HuggingFace models:', hfSearchResults.value.length)
  } catch (error) {
...
async function downloadModelStoreModel(model) {
  ...
  currentDownloadProgress.value = 0
  currentDownloadedSize.value = '0 MB'
  currentTotalSize.value = model.sizeHuman || 'N/A'
  currentSpeed.value = '0.0'
  downloadStatusMessages.value = ['📥 Starte Download...']
  ...
  eventSource.addEventListener('progress', (event) => {
    ...
    if (percentMatch) {
      currentDownloadProgress.value = parseInt(percentMatch[1])
    }
    if (downloadedMatch) {
    ...
    if (speedMatch) {
      currentSpeed.value = parseFloat(speedMatch[1]).toFixed(1)
    }
  })
