      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl h-[80vh] flex flex-col">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h3 class="text-xl font-bold text-gray-900 dark:text-white">
              {{ t('testExpert.title', { name: expert?.name }) }}
            </h3>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
          <div v-if="messages.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-8">
            {{ t('testExpert.askQuestion', { name: expert?.name }) }}
          </div>
          ...
          <div v-if="isLoading" class="mr-auto bg-gray-100 dark:bg-gray-700 p-4 rounded-2xl">
            <div class="flex items-center gap-2 text-gray-500">
              ...
              <span>{{ t('testExpert.thinking', { name: expert?.name }) }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <div class="flex items-center gap-2 mb-3">
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ t('testExpert.mode') }}:</span>
            <select
              v-model="selectedMode"
              class="px-3 py-1 text-sm bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg"
            >
              <option value="">{{ t('testExpert.autoDetect') }}</option>
          ...
          <div class="flex gap-3">
            <input
              ...
              type="text"
              :placeholder="t('testExpert.inputPlaceholder')"
              :disabled="isLoading"
...
async function sendMessage() {
  ...
  try {
    ...
    error(err.response?.data?.error || t('testExpert.requestError'))

    // Add error message
    messages.value.push({
      role: 'assistant',
      content: t('testExpert.errorOccurred')
    })
  } finally {
