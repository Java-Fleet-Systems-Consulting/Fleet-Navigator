<template>
  <div class="fleet-mate-monitor space-y-4">
    ...
    ">
      ...
      <div class="text-sm font-medium text-gray-400 mb-1">{{ t('fleetMate.noMatesConnected') }}</div>
      <div class="text-xs text-gray-500">
        {{ t('fleetMate.noMatesDescription') }}
      </div>
    </div>
    ...
    <div v-for="mate in mates" :key="mate.mateId" class="space-y-4">
      ...
      ">
        ...
        {{ t('fleetMate.loadingHardwareData') }}
      </div>
    </div>
    ...
    >
      ...
      <span>{{ isRefreshing ? t('fleetMate.refreshing') : t('fleetMate.refresh') }}</span>
    </button>
  </div>
</template>
