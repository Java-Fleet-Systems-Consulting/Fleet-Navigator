    ...
    <div v-if="isGenerating" class="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100]">
      <div class="text-center">
        ...
        <p class="text-lg text-gray-300 mb-8">
          {{ t('documentAgent.generatingDocument') }}
        </p>
        ...
        <div class="max-w-lg mx-auto bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
          <p class="text-sm text-gray-300 mb-3">
            💡 <strong class="text-white">{{ t('documentAgent.tipTitle') }}:</strong> {{ t('documentAgent.tipContent') }}
          </p>
          <p class="text-xs text-gray-400">
            {{ t('documentAgent.waitInfo') }}
          </p>
        </div>
      </div>
    </div>
      ...
      >
        ...
        ">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('documentAgent.title') }}</h2>
              <p class="text-sm text-gray-600 dark:text-gray-400">{{ t('documentAgent.subtitle') }}</p>
            </div>
          </div>
          <button
            @click="close"
          ...
          <div v-if="letterTemplates.length > 0" class="mb-6">
            <div class="flex items-center justify-between mb-3">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                📝 {{ t('documentAgent.templateSelect') }} ({{ filteredTemplates.length }})
              </label>
            </div>
            ...
            <div class="mb-3 space-y-2">
              <input
                ...
                type="text"
                :placeholder="t('documentAgent.searchTemplates')"
                class="
            ...
            <div class="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              ...
              <div v-if="filteredTemplates.length === 0" class="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p class="text-sm font-medium">{{ t('documentAgent.noTemplatesFound') }}</p>
                <p class="text-xs mt-1">{{ t('documentAgent.tryDifferentSearch') }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="mb-6">
            <div class="flex items-center justify-between mb-2">
              ...
              >
                ...
                {{ t('documentAgent.saveAsTemplate') }}
              </button>
            </div>
            <textarea
              v-model="prompt"
              :disabled="isGenerating"
              rows="6"
              :placeholder="t('documentAgent.promptPlaceholder')"
              class="
            ...
            <div class="flex items-start gap-2">
              ...
              <div class="flex-1">
                <p class="text-sm font-medium text-green-900 dark:text-green-100">
                  {{ t('documentAgent.agentWorksAutonomous') }}
                </p>
                <p class="text-xs text-green-700 dark:text-green-300 mt-1">
                  {{ t('documentAgent.modelConfiguredInSettings') }}
                </p>
              </div>
            </div>
          </div>
            ...
            ">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-green-700 dark:text-green-300 mt-1">
                    {{ t('documentAgent.documentCreated') }}
                  </p>
                </div>
              </div>
            </div>
        ...
        ">
          <button
            ...
            "
          >
            {{ isGenerating ? t('documentAgent.running') : t('common.cancel') }}
          </button>
          <button
          ...
          >
            ...
            {{ isGenerating ? t('documentAgent.creatingDocument') : t('documentAgent.createDocument') }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showSaveTemplateDialog" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
      ...
      ">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ t('documentAgent.saveTemplateTitle') }}</h3>
          <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
            {{ t('documentAgent.saveTemplateSubtitle') }}
          </p>
        </div>
        ...
        <div class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {{ t('documentAgent.templateName') }} *
            </label>
            <input
              v-model="newTemplate.name"
              type="text"
              :placeholder="t('documentAgent.templateNamePlaceholder')"
              class="
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {{ t('documentAgent.templateDescription') }}
            </label>
            <input
              v-model="newTemplate.description"
              type="text"
              :placeholder="t('documentAgent.templateDescriptionPlaceholder')"
              class="
                w-full px-4 py-2 rounded-lg
                border border-gray-300 dark:border-gray-600
                bg-white dark:bg-gray-700
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {{ t('documentAgent.templateCategory') }}
            </label>
            <input
              v-model="newTemplate.category"
              type="text"
              :placeholder="t('documentAgent.templateCategoryPlaceholder')"
              class="
                w-full px-4 py-2 rounded-lg
                border border-gray-300 dark:border-gray-600
                bg-white dark:bg-gray-700
...
// Setze prompt wenn initialPrompt übergeben wird
...
const loadTemplate = (template) => {
  ...
  successToast(t('documentAgent.templateLoaded', { name: template.name }))
}
...
const saveTemplate = async () => {
  try {
    ...
    successToast(t('documentAgent.templateSaved'))
    await loadTemplates()
    ...
    errorToast(t('documentAgent.templateSaveError'))
  }
}
...
const generateDocument = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      successToast(t('documentAgent.documentSuccess'))
      ...
      errorToast(t('documentAgent.documentError'))
    }
  } catch (error) {
