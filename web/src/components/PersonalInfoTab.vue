  ...
  <div class="space-y-6">
    ...
    <div class="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50">
      <div class="flex items-start gap-2">
        ...
        <div class="text-xs text-blue-800 dark:text-blue-200">
          <p class="font-semibold mb-1">{{ t('personalInfo.header') }}</p>
          <p>{{ t('personalInfo.headerDescription') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-between items-center pt-4">
      ...
      >
        ...
        {{ t('personalInfo.deleteData') }}
      </button>
      <div v-else></div>

      <div class="text-xs text-gray-500 dark:text-gray-400">
        {{ hasData ? t('personalInfo.dataExists') : t('personalInfo.noDataYet') }}
      </div>
    </div>
  </div>
</template>
...
async function savePersonalInfo() {
  try {
    ...
    errorToast(t('personalInfo.saveError'))
    throw error
  }
}
...
async function deleteInfo() {
  const confirmed = await confirm({
    title: t('personalInfo.deleteConfirmTitle'),
    message: t('personalInfo.deleteConfirmMessage'),
    type: 'danger',
    confirmText: t('personalInfo.deleteConfirmButton')
  })
  if (confirmed) {
    try {
      ...
      success(t('personalInfo.deleted'))
    } catch (error) {
      console.error('Failed to delete personal info:', error)
      errorToast(t('personalInfo.deleteError'))
    }
  }
}
</script>
